...
1. Write the ISO (`nix build .#nixie-iso`) to a USB stick and boot it with
   UEFI. The machine's screen shows the wizard; the console also prints a URL
   like `https://<address>:9443/`, a six-digit pairing code and the
   certificate fingerprint, with a QR code, for a browser on the same network.
...
4. Install. Phases 1 to 3 run with streamed output: hardware facts, host
   identity and secrets, partition and install. Reboot.
